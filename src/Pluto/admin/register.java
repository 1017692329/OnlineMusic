...
public class register extends ActionSupport{
	...
	public String execute() throws SQLException,IOException{
		...
		ResultSet rs=conn.executeQuery("select * from admins where name='"+userName+"'");
		if(rs.next())
		{
		...
		{
			boolean insert=conn.execute("insert into admins(Name,Pwd) values('"+userName+"','"+Functions.MD5Encode(userPwd)+"')");
			if(insert)
			{
